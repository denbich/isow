  ...
  <main class="main-content position-relative border-radius-lg ">
        ...
        <div class="container-fluid py-1 px-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
              ...
              <li class="breadcrumb-item text-sm text-white" aria-current="page">{{ __('Kategoria') }}</li>
    ...
    <div class="container-fluid py-4 min-vh-100">
        <div class="row">
         ...
         <div class="card">
            <div class="card-header"><h5 class="mb-0">Edytuj kategorię</h5></div>
              <div class="card-body">
                  <div class="row justify-content-center">
                      <div class="col-lg-8">
                        <form action="{{ route('c.formcategory.update', [$category->ivid]) }}" method="post">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="icon" id="input-icon" value="{{ $category->icon }}">
                            <div class="row pt-2">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label for="name">{{ __('Nazwa kategorii') }}</label>
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" max="255" value="{{ $category->form_category_translate->name }}" required>
                                      @error('name')
                                ...
                                <div class="col-lg-6">
                                  <div class="form-group">
                                      ...
                                      <input type="text" class="form-control @error('description') is-invalid @enderror" name="description" id="description"  value="{{ $category->form_category_translate->description }}" max="255">
                                  @error('description')
                                        <div class="text-danger w-100 d-block">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <select name="color" id="category-color" class="form-control @error('color') is-invalid @enderror">
                                            <option value="primary" @selected($category->color == "primary")>{{ __('Niebieski') }}</option>
                                            <option value="dark" @selected($category->color == "dark")>{{ __('Czarny') }}</option>
                                            <option value="success" @selected($category->color == "success")>{{ __('Zielony') }}</option>
                                            <option value="info" @selected($category->color == "info")>{{ __('Żółty') }}</option>
                                            <option value="danger" @selected($category->color == "danger")>{{ __('Czerwony') }}</option>
                                            <option value="warning" @selected($category->color == "warning")>{{ __('Pomarańczowy') }}</option>
                                        </select>
                                        @error('color')
                                        <div class="text-danger w-100 d-block">
                                            {{ $message }}
                            ...
                            <button class="btn btn-success w-100 my-2" id="create-button-category" type="submit">Zaktualizuj kategorię</button>
                        </form>
                      </div>
                  </div>
              </div>
...
Swal.fire({
    icon: 'success',
    title: 'Kategoria formularza została edytowana pomyślnie!',
...
<script>
    ...
    var current_color = "{{ $category->color }}";
    var category_icon = "{{ $category->icon }}";
    $(document).ready(function(){
        $(".icon-button").addClass("btn-outline-"+current_color);
        $("#button-choice-"+category_icon).addClass("btn-"+current_color);
    });
