...
@section('title') {{ __('Przydzielanie stanowisk') }} @endsection
...
<div class="min-height-300 bg-primary position-absolute w-100" id="background-color-div"></div>
  ...
  <main class="main-content position-relative border-radius-lg ">
        ...
        <div class="container-fluid py-1 px-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
              ...
              <li class="breadcrumb-item text-sm text-white active" aria-current="page">{{ __('Przydzielanie stanowisk') }}</li>
    ...
    <div class="container-fluid py-4 min-vh-100">
        <form action="{{ route('c.form.positions', [$form->ivid]) }}" method="post" id="sign_form" class="h-100">
            @csrf
            <div class="row">
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="mb-0">Przydziel stanowiska </h5>
                </div>
                        ...
                        <table class="table align-items-center mb-0 h-100">
                          <thead>
                            <tr>
                              ...
                              <th class="text-center text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Wybierz stanowisko</th>
                            </tr>
                          </thead>
                          <tbody>
                                ...
                                <td class="align-middle text-center">
                                    <select name="{{ $sign->id }}" id="volunteer-select{{ $sign->id }}" class="form-control">
                                        @foreach ($form_positions as $position)
                                            <option value="{{ $position->id }}" @selected($position->id == $sign->position->id)>{{ $position->form_position_translate->title }}</option>
                                        @endforeach
                                    </select>
                                </td>
                              </tr>
