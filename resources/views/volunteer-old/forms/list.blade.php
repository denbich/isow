    ...
    <div class="container-fluid mt--6">
        ...
        <div>
          ...
          <div class="row">
                        ...
                        <div class="col-xl-3 col-lg-4 col-md-6 mb-4 h-100">
                            <a href="{{ route('v.form.show', [$form->ivid]) }}">
                                <div class="card">
                                    ...
                                    <h4>{{ __('volunteer.form.list.category') }}: <a href="#">{{ $form->category->name }}</a></h4>
                                    <h4><i class="fas fa-users"></i> <span class="name mb-0 text-sm badge badge-primary">{{ $form->signed_form_count }}</span></h4>
                                    <h4>{{ __('volunteer.form.list.date') }}: <br>{{ date('d.m.Y', strtotime($form->expiration))."r. ".date(' H:i', strtotime($form->expiration)) }}</h4>
                                    <h4 class="text-muted text-center">
                                        @php $signed = App\Models\Form_sign::where('form_id', $form->id)->where('volunteer_id', Auth::id())->first(); @endphp
                      ...
                      @empty <div class="card w-100">
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                            <h1 class="text-center text-danger w-100">{{ __('volunteer.form.list.err') }}</h1>
                        </div>
                    </div> @endforelse
          </div>
          @else
